	...
	public void render(IDrawOperation op) {
		...
		for(int i=dist-1;i>=0;i--){
			...
			if(pos != 0){
				...
				if(!d){
					this.drawWAll(Color.red, op, WALL_DROITE, 0, 0, longueur_mur * i);
					this.drawWAll(Color.red, op, WALL_HAUT, largeur_mur, 0, longueur_mur * i);
				}else {
				...
				if(!g) {
					this.drawWAll(Color.red, op, WALL_GAUCHE, 0, 0, longueur_mur * i); 
					this.drawWAll(Color.red, op, WALL_HAUT, -largeur_mur, 0, longueur_mur * i);
				}else {
					this.drawWAll(Color.red, op, WALL_HAUT, -largeur_mur, 0, longueur_mur * (i+1));
					this.drawWAll(Color.gray, op, GROUND, -largeur_mur, hauteur_mur, longueur_mur * i);
